                                     ...
                                     std::vector<float> &int_coords)
{
    ...
    /* `device_time` is a device-side array that stores the
     * intermediate data generated by the intersectBlock kernel.
    ...
    CudaErrorNoCode();*/
    /* `intersect` is a device-side array that stores the coordinates
     * of the intersection points calculated by the intersectBlock kernel.
    ...
    CudaErrorNoCode();*/
    /* `simp_times` is a device-side array that stores the simplified
     * time data originally stored in `device_time`.
     * The default value of its data is -5.
     */
    /*float *simp_times;
    ...
    CudaErrorNoCode();*/
    /* These two vectors are resized so that they can store the contents
     * of `simp_times` and `intersect` respectively.
     */
    ...
    CudaErrorNoCode();*/
    /* Copies the contents of `simp_times` and `intersect` into
     * `host_time` and `int_coords` respectively.
     */
    ...
    fout.close();
    // Frees up the device memory allocated above.
                                     ...
                                     std::vector<float> &sites)
{
    ...
    calcScatteringSites<<<numBlocks, blockSize>>>(b->getX(), b->getY(), b->getZ(), ts, inters, pos, state, N);
    CudaErrorNoCode();
